...
bool
VirtualTrackball::middleButtonPressed( const MouseEvent *event )
{
/*
...
bool
VirtualTrackball::middleButtonMoved( const MouseEvent *event )
{
    ...
    */
    ...
    if( !event->isMiddleButtonPressed() ) return false;
    const int x0 = this->_oldx;
    const int y0 = this->_oldy;
    const int x1 = event->x();
    const int y1 = event->y();
    this->_oldx = x1;
    this->_oldy = y1;
    int dx = x1 - x0;
    int dy = y1 - y0;
    Eigen::Vector3f v( (float)dx , (float)dy , 0.0 );
    v.normalized();
    Eigen::Quaternionf q = this->_model.getCamera().getRotation();
    Eigen::Quaternionf invQ = q.inverse();
    //Eigen::Vector3f v2 = Eigen::Matrix3f( invQ.matrix() ) * v;

    this->_model.moveCenter( v );
...
bool
VirtualTrackball::middleButtonReleased( const MouseEvent *event )
{
    event->x();
    return true;
}
