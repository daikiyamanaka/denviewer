#include "VirtualPan.hpp"

VirtualPan::VirtualPan(Model& model, const float radius, const float epsilon ) : _model ( model ), _radius ( radius ), _eps ( epsilon )
{
        return;
}

VirtualPan::~VirtualPan(void)
{
    return;
}

void
VirtualPan::setSize(const int width, const int height )
{
        this->_width = width;
        this->_height = height;
}
...
bool
VirtualPan::mousePressed ( const MouseEvent* event )
{
...
bool
VirtualPan::mouseMoved ( const MouseEvent* event )
{
        if ( !event->isLeftButtonPressed() ) return false;
...
bool
VirtualPan::mouseReleased ( const MouseEvent* event )
{
        event->x();
        return true;
}
...
Eigen::Vector3f
VirtualPan::project_on_sphere ( const int x, const int y ) const
{
